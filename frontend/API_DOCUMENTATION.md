...
- **请求 Body**:
  ...
  ```
  *(注:* *`password`* *可选，不填或为空表示不修改密码；`role`* *可选，仅管理员有权限修改角色的数据)*
- **响应 Body**:
  ```json
  {
    "code": 200,
...
- **响应 Body (重复预约失败)**:
  ...
  {
    "code": 400,
    "message": "《Java编程思想》已经预约过了，不能重复预约",
    "data": null
  }
  ```
...
- **请求 Body**:
  ```json
  {
    "username": "test_user",
    "title": "Java编程思想",
    "borrowDate": "2026-03-30T10:00:00.000+00:00",
    "status": "BORROWED"
  }
  ```
  *(注:* *`borrowDate`和`status`是可选的，如果不传则默认为当前时间和`BORROWED`)*
- **响应 Body (库存不为0时成功)**:
  ```json
  {
    "code": 200,
...
- **响应 Body (重复借阅失败)**:
  ...
  {
    "code": 400,
    "message": "《Java编程思想》已借阅且未归还，不可重复借阅",
    "data": null
  }
  ```
- **响应 Body (库存为0时)**:
...
- **请求 Body**:
  ```json
  {
    "username": "test_user",
    "title": "Java编程思想",
    "reserveDate": "2026-03-30T10:00:00.000+00:00",
    "status": "RESERVED"
  }
  ```
  *(注:* *`reserveDate`和`status`是可选的，如果不传则默认为当前时间和`RESERVED`)*
- **响应 Body (创建成功)**:
  ```json
  {
    "code": 200,
