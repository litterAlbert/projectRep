  ...
  <div class="knowledge-manage">
    ...
    <div class="table-container">
      <el-table :data="docList" style="width: 100%" v-loading="loading" border stripe>
        ...
        <el-table-column prop="fileUrl" label="文档链接" min-width="200" show-overflow-tooltip>
        ...
        <el-table-column prop="uploaderRole" label="角色" width="100" align="center">
          <template #default="{ row }">
            <el-tag :type="row.uploaderRole === 'admin' ? 'danger' : 'info'" size="small">
              {{ row.uploaderRole === 'admin' ? '管理员' : '普通用户' }}
            </el-tag>
          </template>
        </el-table-column>
...
const loadDocs = async () => {
  loading.value = true
  try {
    const res = await request.get('/ai/document/list')
    if (res.code === 200) {
      docList.value = res.data || []
    } else {
      ElMessage.error(res.message || '加载文档列表失败')
    }
  } catch (e) {
...
const handleDelete = async (row) => {
  try {
    ...
    // 调用删除 API
    const res = await request.delete('/ai/document', {
      params: {
        fileUrl: row.fileUrl,
