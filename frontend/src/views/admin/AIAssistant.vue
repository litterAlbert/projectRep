    ...
    <div class="assistant-layout">
      ...
      <section class="chat-pane">
        ...
        <div class="chat-stream" ref="chatStreamRef">
          <div v-for="msg in messages" :key="msg.id" class="bubble" :class="msg.role">
            <template v-if="msg.chartData">
              <div ref="chartRefs" class="chart-container" :data-id="msg.id" style="width: 100%; height: 300px;"></div>
            </template>
            <template v-else>
              {{ msg.content }}
            </template>
        ...
        <div class="chat-input">
          <el-input v-model="inputText" placeholder="输入问题，支持文档问答与数据分析..." @keyup.enter="sendMessage"></el-input>
...
const messages = ref([
  { id: 1, role: 'user', content: '请分析 2026 年第一季度借阅量最高的前十本书。' },
  { id: 2, role: 'ai', content: '已识别为热门借阅榜查询，可生成柱状图并返回结构化数据。' }
])
...
const renderChart = (msgId, chartData) => {
  nextTick(() => {
    const el = chartRefs.value.find(node => node.dataset.id == msgId)
    if (el) {
      ...
      const option = {
        ...
        xAxis: {
          type: 'category',
          data: chartData.data.map(item => item.username || item.title || item.name)
        },
        yAxis: { type: 'value' },
        series: [{
          type: chartData.chartType === 'bar' ? 'bar' : (chartData.chartType === 'line' ? 'line' : 'pie'),
          data: chartData.data.map(item => item.borrow_count || item.value)
        }]
      }
...
const sendMessage = async () => {
  ...
  try {
    // 简单判断是否是分析意图
    if (query.includes('分析') || query.includes('图表') || query.includes('前十') || query.includes('统计')) {
      const res = await request.post('/ai/analysis', { query })
      const aiMsg = { id: Date.now(), role: 'ai', content: '图表已生成', chartData: res.data }
      messages.value.push(aiMsg)
      renderChart(aiMsg.id, res.data)
    } else {
      const res = await request.post(`/ai/chat/${currentSession.value.id}`, { message: query })
      messages.value.push({ id: Date.now(), role: 'ai', content: res.data })
    }
  } catch (error) {
    // 模拟回复
    setTimeout(() => {
      if (query.includes('分析')) {
        const mockData = {
          chartType: 'bar',
          data: [{ username: 'user1', borrow_count: 5 }, { username: 'user2', borrow_count: 3 }]
        }
        const aiMsg = { id: Date.now(), role: 'ai', content: '图表生成', chartData: mockData }
        messages.value.push(aiMsg)
        renderChart(aiMsg.id, mockData)
      } else {
